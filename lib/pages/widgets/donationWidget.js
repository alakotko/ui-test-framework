...
export class DonationWidget {
  ...
  async fiatDonateScreenActive() {
    const fiatDonateForm = await this.frameLocator.getByTestId(
      "fiat-donate-form"
    );
    await expect(fiatDonateForm).toBeVisible();
  }

  async paymentMethodScreenActive() {
    const paymentMethodScreen = await this.frameLocator.getByTestId(
      "active-screen-payment-method"
    );

    await expect(paymentMethodScreen).toBeVisible();
  }
  async creditCardScreenActive() {
    const activeScreenCreditCard = await this.frameLocator.getByTestId(
      "active-screen-credit-card"
    );

    await expect(activeScreenCreditCard).toBeVisible();
  }

  async privacyScreenActive() {
    const privacyScreen = await this.frameLocator.getByTestId(
      "active-screen-privacy"
    );
